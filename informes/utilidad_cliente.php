<?php
	...
	$pdf->SetY(25);
	$pdf->SetX(60);
	$pdf->Cell(70,6,'Utilidad por Cliente');
	//Line break	
	$pdf->Ln();
	...
	$pdf->SetY(40);
	$pdf->SetX(10);
	$pdf->Cell(70,6,'Fecha Inicio: '.$fecha_inicio);

	//Line break	
	$pdf->Ln();
	$pdf->SetY(46);
	$pdf->SetX(10);
	$pdf->Cell(70,6,'Fecha Fin: '.$fecha_fin);
	...
	$detalle_header=array('Cod. Cliente','Cliente','Total Ventas','Total Ventas s/IVA','Total Costo s/IVA','% Util.','Cant.Fact.');
	$w=array(30,80,40,40,40,20,20);
	//Line break	
	$pdf->Ln();
    ...
    foreach($total_cliente as $detalle) {
		...
		$pdf->Cell(40,6,number_format(floatval($total_factura),$decimals,",","."),1,0,'R');
		$pdf->Cell(40,6,number_format(floatval($total_factura_sin_iva),$decimals,",","."),1,0,'R');
		$pdf->Cell(40,6,number_format(floatval($costo_general),$decimals,",","."),1,0,'R');
		$font_size = 10;
		$pdf->SetFont('Arial','B',$font_size);
	...
	$font_size = 11;
	$pdf->SetFont('Arial','B',$font_size);
	$pdf->Cell(110,6,'Total General Gs.',1,0,'C');
	$pdf->Cell(40,6,number_format(floatval($total_general_factura),$decimals,",","."),1,0,'R');
	$pdf->Cell(40,6,number_format(floatval($total_general_sin_iva),$decimals,",","."),1,0,'R');
	$pdf->Cell(40,6,number_format(floatval($total_general_costo),$decimals,",","."),1,0,'R');
	$pdf->Cell(20,6,number_format(floatval((($total_general_sin_iva/$total_general_costo)-1))*100,2,",","."),1,0,'R');
