<?php
	...
	$result = $productos->get_productos("select CFC.nr, CFC.fecha_factura, CFC.nr_factura, CFC.nr_proveedor, CFC.nr_moneda, 
		CFC.total_factura, P.descripcion, M.id_moneda, M.descripcion descripcion_moneda, CFC.cotizacion_compra
		from cabecera_factura_compra CFC join moneda M on CFC.nr_moneda = M.nr
		join proveedor P on CFC.nr_proveedor = P.nr
		where CFC.fecha_factura >= '".$fecha_inicio."' and CFC.fecha_factura <= '".$fecha_fin."' order by CFC.fecha_factura");
	...
	$pdf->SetY(25);
	$pdf->SetX(60);
	$pdf->Cell(70,6,'Lista de Factura Compra');
	...
	$header=array('#','Fecha','Nro. Factura','Proveedor', 'Monto Factura', 'Mon.');
	$w=array(10,25,35,75,30,15);
	//Line break	
	$pdf->Ln();
	...
	foreach($result as $row) {
		...
		$pdf->Cell(75,6,$row['descripcion'],1,0,'L');
	...
	$font_size = 11;
	$pdf->SetFont('Arial','B',$font_size);
	$pdf->Cell(145,6,'Total General Gs.',1,0,'C');
