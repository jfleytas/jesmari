<?php
	...
	$factura_contado = $_GET['factura_contado'];
	$cliente = $_GET['cliente'];
	$vendedor = $_GET['vendedor'];
	$texto = "Filtrar solo Facturas Contado: ";
	...
	if($factura_contado==1)
    {
    	$sql = $sql . " and FV.fecha_factura = FV.fecha_vto ";	
		$texto= $texto."Si";
    }else{
		$texto= $texto."No";
    }
	...
	$pdf->SetY(25);
	$pdf->SetX(110);
	$pdf->Cell(70,6,'Cuentas a Cobrar');
	...
	$header=array('Cliente','Razon Social','Fecha Factura','Factura Nro.','Vencimiento','Vend.','Saldo a Cobrar','Moneda');
	$w=array(30,100,25,25,25,15,30,15);
	//Line break	
	$pdf->Ln();
	...
	foreach($result as $row) {
		...
		$pdf->Cell(100,6,$row['razon_social'],1,0,'L');
		$pdf->Cell(25,6,$fecha,1,0,'C');
		$pdf->Cell(25,6,$row['nr_factura'],1,0,'R');
		$pdf->Cell(25,6,$fecha_vto,1,0,'C');
		$pdf->Cell(15,6,$vendedor,1,0,'C');
		$pdf->Cell(30,6,number_format(floatval(($row['saldo_factura']*$row['cotizacion_compra'])),$decimals,",","."),1,0,'R');
	...
	$font_size = 10;
	$pdf->SetFont('Arial','B',$font_size);
	$pdf->Cell(180,6,'Total Vencido Gs.',1,0,'C');
	$pdf->Cell(85,6,number_format(floatval($total_vencido_gs),$decimals,",","."),1,0,'C');
	//Line break	
	$pdf->Ln();
	$pdf->Cell(180,6,'Total A Vencer Gs.',1,0,'C');
	$pdf->Cell(85,6,number_format(floatval($total_a_vencer_gs),$decimals,",","."),1,0,'C');
	//Line break	
	$pdf->Ln();
	$pdf->Cell(180,6,'Total General Gs.',1,0,'C');
	$pdf->Cell(85,6,number_format(floatval($total_general_gs),$decimals,",","."),1,0,'C');
